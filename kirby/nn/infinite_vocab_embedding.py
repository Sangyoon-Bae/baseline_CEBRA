...
class InfiniteVocabEmbedding(nn.Module):
    ...
    def _hook_vocab_on_load_state_dict(
        ...
        if not self.is_lazy():
            incoming_vocab = state_dict.pop(prefix + "vocab") # pretrained (inhibitory) -> length 1458 (unit embed 기준~)
            
            # incoming_vocab and self.vocab might have the same keys but in a different order
            # missing_words = [word for word in incoming_vocab if word not in self.vocab] # in fine-tuning. 1457
            # if len(missing_words) > 0:
            #     self.finetuning = True
            #     print(f"Vocabulary mismatch detected. {len(missing_words)} new words found, extending vocab...")
            #     # Automatically extend the vocab with new words
            #     self.extend_vocab(missing_words)
            #     incoming_vocab = self.vocab
            #     print('length of self.vocab after extending missing words', len(self.vocab.items()))
            #     print('length of incoming_vocab after extending', len(incoming_vocab))
            ...
            for word, index in list(self.vocab.items()):
                ...
                remap_indices.append(incoming_vocab.pop(word)) # 싹 다 우겨넣음
