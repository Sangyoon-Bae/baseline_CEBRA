...
class Decoder(StringIntEnum):
    ...
    NATURAL_MOVIE_ONE = 30 # Stella added
    # CRE_LINE = 31 # Stella added
...
decoder_registry = {
    ...
    str(Decoder.NATURAL_MOVIE_ONE): DecoderSpec( 
        dim=900,
        target_dim=1,
        target_dtype="long",
        type=OutputType.MULTINOMIAL,
    ...
    str(Decoder.STATIC_GRATINGS): DecoderSpec(
        ...
        target_dtype="long",
        type=OutputType.MULTINOMIAL,
        timestamp_key="static_gratings_trials.timestamps",
        value_key="static_gratings_trials.stimulus_id",
        loss_fn="bce",
    ),
    ...
    str(Decoder.NATURAL_SCENES): DecoderSpec(
        dim=118,  # image classes [0,...,118]
        target_dim=1,
        target_dtype="long",
        type=OutputType.MULTINOMIAL,
        timestamp_key="natural_scenes_trials.timestamps",
        value_key="natural_scenes_trials.stimulus_id",
        loss_fn="bce",
    ),
    ...
    # str(Decoder.CRE_LINE): DecoderSpec(
    #     dim=13, # number of cre-line
    #     target_dim=1,
    #     target_dtype="long",
    #     type=OutputType.MULTINOMIAL,
    #     timestamp_key="drifting_gratings.timestamps", # this might cause future problem, I guess -> yes it did :(
    #     value_key="drifting_gratings.orientation",
    #     loss_fn="bce",
    # ),
}
