...
def collate(batch):
    ...
    if len(batch) == 0:
        return None  # 또는 raise Exception / skip
    
    # # 2. stack 불가능한 Tensor가 섞여 있는 경우 방어 코드
    # def safe_tensor(x):
    #     if isinstance(x, torch.Tensor):
    #         return x.clone().detach()  # 메모리 공유/resize 문제 방지
    #     return x

    # batch = [{k: safe_tensor(v) for k, v in sample.items()} for sample in batch]
