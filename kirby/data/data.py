...
class ArrayDict(object):
    ...
    def _maybe_first_dim(self):
        ...
        # print('self keys', self.keys) # ['start', 'end']
        if len(self.keys) == 0:
            return None
        else:
    ...
    def __setattr__(self, name, value):
        ...
        if not name.startswith("_"):
            ...
            assert isinstance(
                value, np.ndarray
            ), f"{name} must be a numpy array, got object of type {type(value)} and value is {value}"
            ...
            first_dim = self._maybe_first_dim() # 400번대의 숫자나 3000번대의 숫자.. 모지.. 가끔 None도 들어오네? (dataset 만들 때 그럼.)
            # 이게 갑자기 0, 1, None 이 조합으로 바뀌면 에러가 남!! 0이 되면 안 돼!! (마스크 씌운 이후인가??)
            # print('first dim is', first_dim)
            # print('value shape', value.shape) # always 1.
            if first_dim is not None and value.shape[0] != first_dim:
                print('name', name, 'value', value, 'first dim', first_dim)
                '''
                name train_mask value [ True] first dim 0
                '''
                raise ValueError(
...
class LazyInterval(Interval):
    ...
    def slice(self, start: float, end: float, reset_origin: bool = True):
        ...
        # leap = np.array(out.end) - np.array(out.start)
        # 이 leap 은 [1.999]이던 []이던 상관이 없음..
        '''
        out in slice function LazyInterval(
        end=<HDF5 dataset "end": shape (9000,), type "<f8">,
        frame_number=<HDF5 dataset "frame_number": shape (9000, 1), type "<f4">,
        start=<HDF5 dataset "start": shape (9000,), type "<f8">,
        test_mask=<HDF5 dataset "test_mask": shape (9000,), type "|b1">,
        timestamps=<HDF5 dataset "timestamps": shape (9000,), type "<f8">,
        train_mask=<HDF5 dataset "train_mask": shape (9000,), type "|b1">,
        valid_mask=<HDF5 dataset "valid_mask": shape (9000,), type "|b1">
        )
        '''
        return out
